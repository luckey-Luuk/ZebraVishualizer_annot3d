...
from PySide6.QtWidgets import QApplication, QComboBox, QDockWidget, QFileDialog, QDialog, QHBoxLayout, QInputDialog, QLabel, QMainWindow, QPushButton, QSlider, QSpinBox, QGridLayout, QVBoxLayout, QWidget, QTabWidget, QFrame, QButtonGroup, QScrollArea, QCheckBox, QSpinBox#, QAction
...
class Visualization(HasTraits):
    ...
    def picker_callback(self,picker): #kijkt waar je klikt en tekent punt
        if self.show_volume==True:
            ...
            self.draw_annotation(coordinates[0],coordinates[1],coordinates[2])

    def draw_annotation(self,new_x,new_y,new_z):#updates point data and draws updated point
        #update point data
    ...
    def draw_previous_annotation(self): #places the point in the same location as it was in the previous image number
        if self.current_frame_number!=0 and self.current_point_index in self.point_location_data[self.current_frame_number-1]:
            ...
            self.draw_annotation(new_location[0],new_location[1],new_location[2])

    def add_value_to_annotation(self,added_value):
        if self.current_point_index in self.point_location_data[self.current_frame_number]:
            ...
            self.draw_annotation(old_value[0]+added_value[0],old_value[1]+added_value[1],old_value[2]+added_value[2])
    ...
    def draw_trajectory_step(self, frame, point_index): #tekent trajectory
    ...
    def remove_all_trajectories(self): #stop met trajectory visualiseren
        for p in range(amount_of_points):
    ...
    def toggle_all_trajectories(self): #TODO: maak nieuwe checkbox om alle checkboxes te togglen
    ...
    def goto_frame(self, frame_number):
        if frame_number>self.amount_of_frames-1:
            self.current_frame_number = self.amount_of_frames-1
        elif frame_number<0:
            self.current_frame_number = 0
        else:
    ...
    def update_annot(self): # update the scalar field and visualization auto updates #TODO: in code toevoegen ipv eigen functie?
        npspace = annot3D.get_npspace()
...
class MainWindow(QMainWindow): #hele raam
    ...
    # OPTIONS PAGE
        ...
        continue_button.clicked.connect(self.mayavi_widget.visualization.draw_previous_annotation)
        annotation_buttons_layout.addWidget(continue_button, 0, 1)

        delete_button = QPushButton('delete\nannotation') #'delete' is wat er op knop staat
        delete_button.clicked.connect(self.mayavi_widget.visualization.delete_annotation) #connectie tussen knop en functie
        ...
        def create_adjust_button(self, text, update=[0,0,0]):
            ...
            self.button.clicked.connect(lambda: self.mayavi_widget.visualization.add_value_to_annotation(update))
            return self.button
    ...
    # FRAME NAVIGATION LAYOUT
        ...
        delay_box = QSpinBox(minimum=10, maximum=100000, value=1000, suffix='ms')
        delay_box.setSingleStep(10)
        delay_box.valueChanged.connect(self.change_delay)
        navigation_layout.addWidget(delay_box, 1, 0)
    ...
    # HIDDEN HOTKEY ACTIONS
        ...
        slideLeftAction.triggered.connect(self.change_volume_model_previous)
        ...
        renderAction.triggered.connect(self.mayavi_widget.visualization.update_annot)
    ...
    def goto_frame(self): #'goto' knop popup
        ...
        i, ok = QInputDialog.getInt(self, "Frame selection", "Go to frame:", 0, 0, self.mayavi_widget.visualization.amount_of_frames-1)
        if ok: # current frame cs must be a number
            window.mayavi_widget.visualization.goto_frame(i)
            self.update_frame_number()

    def change_volume_model_next(self): #TODO: in code toevoegen ipv eigen functie?
        window.mayavi_widget.visualization.next_frame()
        self.update_frame_number()

    def change_volume_model_previous(self): #TODO: in code toevoegen ipv eigen functie?
        window.mayavi_widget.visualization.previous_frame()
        self.update_frame_number()

    def change_transparancy(self):
        window.mayavi_widget.visualization.transparancy = self.transparency_slider.value()/10
        window.mayavi_widget.visualization.update_volume()

    def change_sphere_size(self):
        window.mayavi_widget.visualization.sphere_size = self.sphere_size_slider.value()
        window.mayavi_widget.visualization.redraw_all_points()
